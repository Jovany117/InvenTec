...
if ($_POST['captcha'] != $_SESSION['captcha']) {
    ...
    echo "<script>alert('Captcha incorrecto'); window.location.href = 'login_ventas.html';</script>";
    exit;
}
...
$sql = "SELECT * FROM usuarios_ventas WHERE username='$username'";
...
if ($result->num_rows > 0) {
    ...
    if (password_verify($password, $hashedPassword)) {
        ...
        echo "<script>window.location.href = 'login_ventas.html';</script>";
        exit(); 
    }
} else {
    logEvent($username, "Intento de inicio de sesión fallido", "Intento fallido de iniciar sesión para el usuario: $username");
    echo "<script>alert('Nombre de usuario o contraseña incorrectos'); window.location.href = 'login_ventas.html';</script>";
}
?>
